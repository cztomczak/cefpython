...
bool ClientHandler::OnLoadError(
  ...
  REQUIRE_UI_THREAD();
  return LoadHandler_OnLoadError(browser, frame, errorCode, const_cast<CefString&>(failedUrl), errorText);
}
...
bool ClientHandler::OnKeyEvent(
  ...
  REQUIRE_UI_THREAD();
  return KeyboardHandler_OnKeyEvent(browser, eventType, keyCode, modifiers, isSystemKey, isAfterJavascript);
}
